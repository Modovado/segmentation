...
class SatelliteDataset(Dataset):
    def __init__(self,
        ...
        # [absolute path]
        self.image_list = sorted(glob.glob(self.image_dir + '*.tif'), key=len)
        self.label_list = sorted(glob.glob(self.label_dir + '*.png'), key=len)  # geoseg
    ...
    def load_image_and_label(self, index):
        ...
        # load image and mask
        # image
        image = tifffile.imread(self.image_list[index]).astype('int16')
